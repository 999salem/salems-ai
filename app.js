(() => {
  ...
  async function handleMessage(text) {
    ...
    if (googleQuery) {
      ...
      if (!navigator.onLine) {
        addChat('assistant', 'Network offline. I can queue the search, but Google needs an internet connection.');
        setCoreState('offline', 'NETWORK OFFLINE');
        renderAll();
        return;
      }

      // Keep this synchronous with the user gesture so iOS permits the new tab.
      const opened = window.open(url, '_blank', 'noopener,noreferrer');
      addChat('assistant', `Searching Google for “${googleQuery}”. ${opened ? 'I opened the results for you.' : 'Tap the Google source below to open the results.'}`, [
  ...
  function renderSystemStatus() {
    ...
    $('#searchStatus').textContent = !navigator.onLine ? 'OFFLINE' : (state.profile?.backendUrl ? 'AI + WEB' : 'GOOGLE READY');

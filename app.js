(() => {
  ...
  async function init() {
    ...
    const boot = $('#boot');
    if (state.settings?.bootAnimation !== false) {
  ...
  function bindUI() {
    ...
    $('#chatForm').addEventListener('submit', e => { e.preventDefault(); submitInput($('#chatInput')); });
    $('#chatFormFull').addEventListener('submit', e => { e.preventDefault(); submitInput($('#chatInputFull')); });
    ...
    $('#installCardBtn')?.addEventListener('click', openInstallGuide);
    $('#installHelpBtn')?.addEventListener('click', openInstallGuide);
    $('#closeInstallSheet')?.addEventListener('click', closeInstallGuide);
    $('#installSheet')?.addEventListener('click', e => { if (e.target === $('#installSheet')) closeInstallGuide(); });
  }
  ...
  async function handleMessage(text) {
    ...
    if (state.profile?.backendUrl) {
      ...
      const reply = currentInfo
        ? `That needs live information. Your local S.A.L.E.M. systems are working, but internet AI/search isn't connected yet. Add a secure backend URL in Profile and I can route current questions through it.`
        : `I can handle your saved life data locally. For open-ended AI questions, connect a secure backend in Profile. Never put an API key directly in this app.`;
      addChat('assistant', reply);
      setCoreState('idle', navigator.onLine ? 'LOCAL MODE' : 'NETWORK OFFLINE');
      renderAll();
    }
  }
  ...
  async function runLocalCommand(raw) {
    ...
    if (/next shift|when do i work|work tomorrow|shift tomorrow/.test(t)) {
  ...
  async function applyBackendActions(actions) {
    for (const action of actions) {
      ...
      if (action.type === 'createReminder' && action.title) {
        state.reminders.unshift({ id: crypto.randomUUID ? crypto.randomUUID() : String(Date.now()), title: String(action.title), category: action.category || 'personal', priority: action.priority || 'normal', due: action.due || 'Unscheduled', done: false });
      }
  ...
  function renderSystemStatus() {
    ...
    $('#aiStatus').textContent = state.profile?.backendUrl ? 'CONNECTED' : 'LOCAL MODE';
    $('#searchStatus').textContent = state.profile?.backendUrl && navigator.onLine ? 'AVAILABLE' : 'BACKEND NEEDED';
  ...
  function renderProfile() {
    ...
    if ($('#standaloneStatus')) $('#standaloneStatus').textContent = isStandalone() ? 'INSTALLED' : 'WEB MODE';
  }
  ...
  async function saveSettings() {
    ...
    await persist();
    renderAll();
    addChat('assistant', `Settings saved. ${state.profile.backendUrl ? 'Secure backend endpoint configured.' : 'Running in local mode.'}`);
    renderChats();
  }
  ...
  function renderInstallState() {
    ...
    if (btn) {
      ...
      $('#installBtnText').textContent = standalone ? 'INSTALLED' : 'INSTALL';
    }
    if ($('#standaloneStatus')) $('#standaloneStatus').textContent = standalone ? 'INSTALLED' : 'WEB MODE';
  }

  async function openInstallGuide() {
    if (isStandalone()) {
      showToast('S.A.L.E.M. is already installed as a web app.');
      return;
    }
